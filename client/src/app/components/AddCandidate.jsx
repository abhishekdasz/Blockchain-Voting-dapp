...
const AddCandidate = () => {
  ...
  const addNewCandidate = async () => {
    try {
      ...
      const transaction = await contract.addCandidate(name, parseInt(age), party, candidateAddress);
  ...
  const startOrEndVoting = async () => {
    try {
      ...
      if (votingStatus) {
        ...
        const transaction = await contract.startVoting(parseInt(votingDuration));
        await transaction.wait();
  ...
  const getAllCandidatesWithVotes = async () => {
    try {
      ...
      const candidatesWithVotes = await Promise.all(
        Array.from({ length: candidates.length }, (_, i) => contract.getCandidateWithVotes(i))
      );
      ...
      <div>
        ...
        <ul>
          {candidates.map((candidate, index) => (
            <li key={index}>
              Name: {candidate.name}, Age: {candidate.age}, Party: {candidate.party}, Address:{' '}
              {candidate.candidateAddress}, Votes: {candidate.voteCount}
            </li>
          ))}
        </ul>
      </div>
